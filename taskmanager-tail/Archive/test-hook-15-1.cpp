...
// @description     Window Cycling Probe (v15.1)
// @version         15.1
// @author          sb4ssman
...
// ==WindhawkModReadme==
/*
# Test Hook Explorer v15.1 (Window Cycling)
...
**Changes in v15.1:**
- Uses `ShowWindowAsync` to try and bypass UIPI.
- Adds `SetWindowPos` fallback.
- Verifies visibility before proceeding.
...
void CycleWindow(HWND hwnd) {
    ...
    if (!GetWindowPlacement(hwnd, &wp)) {
        Wh_Log(L"GetWindowPlacement failed: %d", GetLastError());
    }
    
    // 2. Hide (Removes button)
    // Try Async first to potentially bypass UIPI
    ShowWindowAsync(hwnd, SW_HIDE);
    Sleep(200);
    
    if (IsWindowVisible(hwnd)) {
        Wh_Log(L"ShowWindowAsync(SW_HIDE) failed. Trying SetWindowPos...");
        // Force hide using SetWindowPos
    ...
    if (IsWindowVisible(hwnd)) {
        Wh_Log(L"Failed to hide window. Aborting cycle. (Is TaskMgr elevated?)");
        return;
    }
...
BOOL Wh_ModInit() {
    Wh_Log(L"=== Test Hook Explorer v15.1 (Window Cycling) ===");
    g_stopThread = false;
