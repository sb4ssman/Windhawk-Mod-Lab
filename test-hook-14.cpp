...
// @description     Virtual Touch Probe (v14.0)
// @version         14.0
// @author          sb4ssman
...
// ==WindhawkModReadme==
/*
# Test Hook Explorer v14.0 (Virtual Touch)

This version attempts to move the Task Manager button using **Touch Injection**.

**Why this is different from Mouse Simulation:**
- It does **NOT** move your mouse cursor.
- It creates a separate "Virtual Finger" input stream.
- It uses the modern `InjectTouchInput` API which XAML supports natively.

**Safety:**
- It waits for 2 seconds of user inactivity before acting.
- It targets the "Sweet Spot" (75% width) to ensure the drop is accepted.
...
bool IsUserIdle() {
    ...
    if (GetLastInputInfo(&lii)) {
        ...
        // Wait for 2 seconds of idle time
        if (tick - lii.dwTime < 2000) return false;
    }
    return true;
}

// Helper to simulate TOUCH drag (Does not move mouse cursor)
void VirtualTouchDrag(POINT start, POINT end) {
    Wh_Log(L"Executing Virtual Touch Drag: (%d, %d) -> (%d, %d)", start.x, start.y, end.x, end.y);
    ...
    contact.pointerInfo.pointerId = 1; // ID 1 = Virtual Finger
    contact.pointerInfo.ptPixelLocation = start;
    contact.pointerInfo.pointerFlags = POINTER_FLAG_DOWN | POINTER_FLAG_INRANGE | POINTER_FLAG_INCONTACT;
    contact.touchFlags = TOUCH_FLAG_NONE;
    contact.touchMask = TOUCH_MASK_CONTACTAREA | TOUCH_MASK_ORIENTATION | TOUCH_MASK_PRESSURE;
    
    // Define contact area (simulating a fingertip)
    ...
    // 3. Hold at destination (Crucial for XAML reorder logic)
    Sleep(300);
    ...
    Wh_Log(L"Virtual Touch Drag Complete.");
}
                ...
                if (pChildren) {
                    ...
                    if (taskMgrIndex != -1 && static_cast<size_t>(taskMgrIndex) < buttons.size() - 1) {
                        if (!IsUserIdle()) {
                            ...
                            Wh_Log(L"Task Manager found at index %d (of %d). Attempting Virtual Touch...", taskMgrIndex, buttons.size());
                            ...
                            // Target: 75% across last button (Sweet Spot)
...
BOOL Wh_ModInit() {
    Wh_Log(L"=== Test Hook Explorer v14.0 (Virtual Touch) ===");
    g_stopThread = false;
