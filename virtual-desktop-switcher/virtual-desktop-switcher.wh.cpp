...
## Settings
- **Position** — anywhere in the system tray, or at the left/right taskbar edge
...
- **Label format** — numbers (1 2 3), dots (● ○ ○), or custom comma-separated labels

## Known limitations (v0.1)
- Buttons do not update when desktops are added/removed without switching. Toggle mod off/on to refresh.
...
- position: "afterClock"
  $name: Position
  $description: Where to inject the VD buttons
  $options:
  ...
  - "taskbarLeft": "Left edge of taskbar"
  - "taskbarRight": "Right of task list, left of tray"
...
static bool InjectButtonGrid(FrameworkElement root) {
    ...
    // Tray positions use SystemTrayFrameGrid; taskbar positions use RootGrid.
    bool useTray = (pos == L"afterClock" || pos == L"beforeClock" ||
                    pos == L"beforeOmni" || pos == L"beforeIcons" ||
                    pos == L"afterShowDesktop");

    FrameworkElement parent = FindChildRecursive(root, [&useTray](FrameworkElement fe) {
        return fe.Name() == (useTray ? L"SystemTrayFrameGrid" : L"RootGrid");
    });
    if (!parent) {
        Wh_Log(L"[Inject] Parent not found for position=%ls", pos.c_str());
        return false;
    }
...
BOOL Wh_ModInit() {
    Wh_Log(L"[Init] VD Switcher v0.1");
    LoadSettings();
    DetectExplorerBuild();
