// ==WindhawkMod==
// @id              vertical-omnibutton2
// @name            Vertical OmniButton2
...
bool g_ixdeStarted = false;
bool g_ixdePending = false; // delayed-inject thread in flight
...
static void InjectOmniButtonTAPDelayed(); // forward declaration
...
public:
    HRESULT STDMETHODCALLTYPE SetSite(IUnknown* pUnkSite) override try {
        ...
        if (pUnkSite) {
            ...
            // SetSite(null) means another XAML diagnostics consumer displaced us.
            // Reset and re-queue injection so we reclaim the slot once it settles.
            g_ixdeStarted = false;
            InjectOmniButtonTAPDelayed();
        }
...
static void InjectOmniButtonTAP(); // forward declaration

// Delay injection so mods that also use XAML diagnostics (e.g. Taskbar Styler
// with xamlDiagnosticsHandling=allow) can initialize first and then yield.
static void InjectOmniButtonTAPDelayed() {
    if (g_ixdeStarted || g_ixdePending) return;
    g_ixdePending = true;
    // Hold a DLL reference so the module isn't freed before the thread wakes up.
    HMODULE hSelf = nullptr;
    GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
        reinterpret_cast<LPCWSTR>(&InjectOmniButtonTAPDelayed), &hSelf);
    HANDLE t = CreateThread(nullptr, 0, [](LPVOID p) -> DWORD {
        HMODULE hMod = reinterpret_cast<HMODULE>(p);
        Sleep(1500);
        if (!g_unloading && !g_ixdeStarted)
            InjectOmniButtonTAP();
        g_ixdePending = false;
        FreeLibrary(hMod);
        return 0;
    }, reinterpret_cast<LPVOID>(hSelf), 0, nullptr);
    if (t) CloseHandle(t);
    else  { g_ixdePending = false; if (hSelf) FreeLibrary(hSelf); }
}
...
HMODULE WINAPI LoadLibraryExW_Hook(LPCWSTR lpLibFileName, HANDLE hFile, DWORD dwFlags) {
    ...
    if (hModule && !g_visualTreeWatcher && lpLibFileName) {
        ...
        if (_wcsicmp(name, L"Windows.UI.Xaml.dll") == 0) {
            Wh_Log(L"[LoadLib] Windows.UI.Xaml.dll loaded — injecting TAP (delayed)");
            InjectOmniButtonTAPDelayed();
        }
    }
...
void Wh_ModAfterInit() {
    ...
    if (!g_ixdeStarted && !g_ixdePending) {
        HMODULE hXaml = GetModuleHandleW(L"Windows.UI.Xaml.dll");
        if (hXaml) {
            Wh_Log(L"[AfterInit] Retrying inject (delayed)...");
            InjectOmniButtonTAPDelayed();
        }
    }
