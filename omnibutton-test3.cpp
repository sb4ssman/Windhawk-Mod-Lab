...
- showBatteryPercent: false
  ...
  $description: Make the battery slot taller to reveal percentage text (requires debug logging to confirm element is present)
- debugLogging: false
...
static void ApplyLayout(StackPanel const& sp) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Battery slot (last child) gets extra height when showBatteryPercent is on.
        bool isBattery = g_settings.showBatteryPercent && (i == n - 1);
...
BOOL Wh_ModInit() {
    Wh_Log(L"[Init] Vertical OmniButton v1.14.0");
    LoadSettings();
