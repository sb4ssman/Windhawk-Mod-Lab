...
- **Show battery percentage** — toggles the Windows native battery % display (requires explorer restart)
...
- showBatteryPercent: false
  ...
  $description: Toggle the Windows native battery percentage display next to the battery icon. Requires restarting explorer.exe to apply.
- clockLineSpacing: 8
...
static void ApplyLayout(StackPanel const& sp) {
    ...
    // Center the icon stack within the button's full height/width.
    ...
    Wh_Log(L"[Layout] Applied vertical layout (children=%d)",
           VisualTreeHelper::GetChildrenCount(sp));
}
...
BOOL Wh_ModInit() {
    Wh_Log(L"[Init] Vertical OmniButton v1.20.0");
    LoadSettings();
...
void Wh_ModUninit() {
    ...
    auto doCleanup = [sp, btn, cdp, timeTB, dateTB]() {
        try {
            if (sp) {
...
void Wh_ModSettingsChanged() {
    ...
    try {
        ...
        [sp, btn, cdp, timeTB, dateTB, enableV, enableC, clockAlign, clockSpacing]()
        {
            // OmniButton layout
            try {
                if (sp) {
