...
// @description     Keyboard Shuffle v12.2 (Verified Focus)
// @version         12.2
// @author          sb4ssman
...
// ==WindhawkModReadme==
/*
# Test Hook Explorer v12.2 (Verified Focus Shuffle)

This version attempts to move the Task Manager button using **Keyboard Shortcuts**.
It includes strict checks to verify the button actually has focus before typing.

**Mechanism:**
1. Finds Task Manager button.
2. Calls `SetFocus()`.
3. **Verifies** `HasKeyboardFocus` property.
4. Sends `Alt` + `Shift` + `Right Arrow`.
...
// Helper to send keystrokes with Scan Codes
...
void KeyboardMove(IUIAutomationElement* pElement, int steps) {
    ...
    Wh_Log(L"  -> Focus confirmed: %s", focused ? L"YES" : L"NO");

    if (!focused) {
        Wh_Log(L"  -> Aborting move because focus could not be obtained.");
        // Restore original focus before leaving
        if (hForeground) SetForegroundWindow(hForeground);
        return;
    }
...
BOOL Wh_ModInit() {
    Wh_Log(L"=== Test Hook Explorer v12.2 (Verified Focus) ===");
    g_stopThread = false;
