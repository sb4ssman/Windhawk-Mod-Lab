...
- cameraHardwareDetection: true
  ...
  $description: >-
    ...
    frame capture. Turn off if a particular camera activates its LED/indicator
    or behaves poorly while monitored.
...
// a preview/frame reader. The initial open/closed state is live-confirmed on
// the Legion without creating a webcam usage record of its own; the persistent
// StateChanged path still needs its live Windhawk test.
...
public:
    ...
    HRESULT Init() {
        if (!g_cameraHardwareDetectionEnabled.load()) {
            Wh_Log(L"[CamMon] Hardware detection disabled by setting");
            return S_FALSE;
        }
    ...
    void Refresh() {
        ...
        if (!m_occlusion) {
            if (m_apiUnsupported)
                return;
            // A software privacy toggle or temporarily unavailable camera can
            // make initialization fail. Retry occasionally so enabling it later
            // doesn't require reloading the mod.
            ULONGLONG now = GetTickCount64();
            if (now - m_lastInitAttempt >= 10000)
                TryInitialize();
            return;
        }

        try {
            UpdateState(m_occlusion.GetState(), L"refresh", m_refreshEvent);
        } catch (winrt::hresult_error const& e) {
            Wh_Log(L"[CamMon] GetState failed hr=0x%08X; reinitializing",
...
private:
    HRESULT TryInitialize() {
        m_lastInitAttempt = GetTickCount64();
        Cleanup();
...
void Wh_ModAfterInit() {
    ...
    g_retryThread = CreateThread(nullptr, 0, [](void* param) -> DWORD {
        ...
        // Phase 2: monitor event-driven hardware state and retain the existing
        // 3-second sweep as a fallback for registry/ConsentStore state.
