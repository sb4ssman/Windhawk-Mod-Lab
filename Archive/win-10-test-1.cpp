...
WinVersion DetectWindowsVersion() {
    ...
    // Build 22000+ is Windows 11, below is Windows 10
    // Build 26100+ is Windows 11 24H2
    if (build < 22000) {
...
// Windows 10 taskbar hierarchy: Shell_TrayWnd -> ReBarWindow32 -> MSTaskSwWClass -> MSTaskListWClass
void CheckAndMoveWin10(IUIAutomation* pAutomation) {
    ...
    // Navigate to MSTaskListWClass - the actual task button container in Win10
    HWND hRebar = FindWindowExW(hTray, NULL, L"ReBarWindow32", NULL);
    if (!hRebar) {
        Wh_Log(L"Win10: ReBarWindow32 not found");
        return;
    }

    HWND hTaskSw = FindWindowExW(hRebar, NULL, L"MSTaskSwWClass", NULL);
    if (!hTaskSw) {
        Wh_Log(L"Win10: MSTaskSwWClass not found");
        return;
    }

    HWND hTaskList = FindWindowExW(hTaskSw, NULL, L"MSTaskListWClass", NULL);
    if (!hTaskList) {
        Wh_Log(L"Win10: MSTaskListWClass not found");
        return;
    }

    // Use UI Automation on the task list
    IUIAutomationElement* pRoot = NULL;
    HRESULT hr = pAutomation->ElementFromHandle(hTaskList, &pRoot);
    if (FAILED(hr) || !pRoot) {
        Wh_Log(L"Win10: Failed to get automation element for task list");
        return;
    }

    // Create condition to find all button-like elements
    ...
    if (pChildren) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (pChild) {
                // In Windows 10, taskbar buttons are direct children of MSTaskListWClass
                // They have names that match the window titles
                CONTROLTYPEID controlType;
                pChild->get_CurrentControlType(&controlType);

                // Check if it's a button (UIA_ButtonControlTypeId = 50000)
                if (controlType == UIA_ButtonControlTypeId) {
                    ...
                    if (item.name && wcsstr(item.name, g_settings.targetName.c_str())) {
                        ...
                        Wh_Log(L"Win10: Found target '%s' at index %d", item.name, targetIndex);
                    }
                }
                pChild->Release();
            }
        ...
        if (targetIndex != -1 && static_cast<size_t>(targetIndex) < buttons.size() - 1) {
            ...
            if (GetTickCount() - g_lastAttemptTime > 1000) {
                Wh_Log(L"Win10: Target found at index %d (of %d). Moving...", targetIndex, (int)buttons.size());
...
// Unified check function that dispatches to version-specific implementation
