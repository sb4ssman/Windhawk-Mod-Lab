...
// @description     Task Manager Tail (v16.2)
// @version         16.2
// @author          sb4ssman
...
void LoadSettings() {
    ...
    if (g_settings.checkInterval < 10) g_settings.checkInterval = 10;
}
                ...
                if (pChildren) {
                    ...
                    if (taskMgrIndex != -1 && static_cast<size_t>(taskMgrIndex) < buttons.size() - 1) {
                        if (!IsUserIdle()) {
                            ...
                            // Find the actual Task Manager window (Reverted to v16.0 method)
...
BOOL Wh_ModInit() {
    Wh_Log(L"=== Test Hook Explorer v16.2 (ITaskbarList + Settings) ===");
    LoadSettings();
    g_stopThread = false;
...
void Wh_ModUninit() {
    Wh_Log(L"=== Test Hook Explorer Uninit (v16.2) ===");
    g_stopThread = true;
    if (g_hThread) {
