...
// Justify, HorizontalTextAlignment Justify, and wrapping. Normal spaces are
// converted to non-justifying spaces and `%s%` is resolved to a normal space.
// Live result: with HorizontalTextAlignment, TextAlignment reads back as
// Justify (3), but the text is left-aligned and `%s%` still doesn't expand.
// Native TextBlock justification doesn't appear usable for the spacer.
...
constexpr PCWSTR kNonJustifyingSpace = L"\u2005";
...
std::wstring FormatLineToString(std::wstring_view format) {
    ...
    if (formatted.find(kSpacerToken) != std::wstring::npos) {
        formatted = ReplaceAll(formatted, L" ", kNonJustifyingSpace);
...
void ApplySpacerJustifiedTextBlock(Controls::TextBlock textBlock,
    ...
    if (usesSpacer && maxWidth > 0) {
        ...
        textBlock.HorizontalTextAlignment(TextAlignment::Justify);
        textBlock.TextWrapping(TextWrapping::Wrap);
        Wh_Log(L"Spacer justify %s: TextAlignment=%d "
               L"HorizontalTextAlignment=%d Width=%.1f MaxWidth=%.1f "
               L"TextWrapping=%d HorizontalAlignment=%d",
               textBlock.Name().c_str(),
               static_cast<int>(textBlock.TextAlignment()),
               static_cast<int>(textBlock.HorizontalTextAlignment()),
               textBlock.Width(), textBlock.MaxWidth(),
               static_cast<int>(textBlock.TextWrapping()),
               static_cast<int>(textBlock.HorizontalAlignment()));
        return;
    }
    ...
    textBlock.as<DependencyObject>().ClearValue(
        FrameworkElement::HorizontalAlignmentProperty());
    textBlock.as<DependencyObject>().ClearValue(
        Controls::TextBlock::HorizontalTextAlignmentProperty());
}
