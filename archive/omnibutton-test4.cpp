...
- showBatteryPercent: false
  $name: Show battery percentage (experimental)
  $description: Make the battery slot taller to reveal percentage text (enable debug logging too, to confirm element is present)
- clockAlignment: 1
  $name: Clock text alignment
...
void LoadSettings() {
    ...
    g_settings.showBatteryPercent = Wh_GetIntSetting(L"showBatteryPercent") != 0;
    g_settings.clockAlignment     = Wh_GetIntSetting(L"clockAlignment");
    if (g_settings.clockAlignment < 0) g_settings.clockAlignment = 0;
    if (g_settings.clockAlignment > 2) g_settings.clockAlignment = 2;
...
static FrameworkElement g_omniItemsPresenter{ nullptr }; // parent of StackPanel
...
static void LogBatterySubtree(DependencyObject const& node, int depth) {
    if (depth > 4) return;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (fe) {
            Wh_Log(L"[Battery]   [%d/%d] class=%s name=%s vis=%d%s",
                depth, i, winrt::get_class_name(fe).c_str(), fe.Name().c_str(),
                (int)fe.Visibility(),
                tb ? (std::wstring(L" text=") + std::wstring(tb.Text().c_str())).c_str() : L"");
        }
        LogBatterySubtree(child, depth + 1);
    }
}
...
static void ApplyLayout(StackPanel const& sp) {
    ...
    // Center the StackPanel within the OmniButton by also centering its parent
    // (ItemsPresenter), which is what actually controls the items region width.
    auto spParent = VisualTreeHelper::GetParent(sp).try_as<FrameworkElement>();
    if (spParent) {
        spParent.HorizontalAlignment(HorizontalAlignment::Center);
        g_omniItemsPresenter = spParent;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Identify the battery slot by walking into the child to find "Battery" in the class.
        bool isBattery = false;
        if (g_settings.showBatteryPercent) {
            int nc = VisualTreeHelper::GetChildrenCount(child);
            for (int j = 0; j < nc && !isBattery; j++) {
                auto grandchild = VisualTreeHelper::GetChild(child, j);
                if (grandchild) {
                    std::wstring cls(winrt::get_class_name(grandchild).c_str());
                    if (cls.find(L"Battery") != std::wstring::npos ||
                        cls.find(L"battery") != std::wstring::npos) {
                        isBattery = true;
                    }
                }
            }
        ...
        child.Height(isBattery ? g_settings.iconHeight * 2.0 : g_settings.iconHeight);
        if (isBattery) {
            Wh_Log(L"[Battery] Battery slot at index %d — logging subtree:", i);
            LogBatterySubtree(child, 0);
        }
    }
...
BOOL Wh_ModInit() {
    Wh_Log(L"[Init] Vertical OmniButton v1.15.0");
    LoadSettings();
...
void Wh_ModUninit() {
    ...
    auto doCleanup = [sp, ip, btn, timeTB, dateTB]() {
        try {
            if (ip) ip.ClearValue(FrameworkElement::HorizontalAlignmentProperty());
        } catch (...) {}
        try {
            if (sp) {
                sp.ClearValue(StackPanel::OrientationProperty());
                sp.ClearValue(FrameworkElement::HorizontalAlignmentProperty());
                int n = VisualTreeHelper::GetChildrenCount(sp);
                for (int i = 0; i < n; i++) {
                    auto child = VisualTreeHelper::GetChild(sp, i).try_as<FrameworkElement>();
                    if (child) {
        ...
        try {
            if (timeTB) timeTB.ClearValue(TextBlock::TextAlignmentProperty());
        } catch (...) {}
        try {
            if (dateTB) {
...
void Wh_ModSettingsChanged() {
    ...
    if (g_omniStackPanel) {
        try {
            if (g_settings.enableVertical) {
                if (g_omniItemsPresenter)
                    g_omniItemsPresenter.HorizontalAlignment(HorizontalAlignment::Center);
                g_omniStackPanel.Orientation(Orientation::Vertical);
                g_omniStackPanel.HorizontalAlignment(HorizontalAlignment::Center);
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (g_settings.showBatteryPercent) {
                        int nc = VisualTreeHelper::GetChildrenCount(child);
                        for (int j = 0; j < nc && !isBattery; j++) {
                            auto gc = VisualTreeHelper::GetChild(child, j);
                            if (gc) {
                                std::wstring cls(winrt::get_class_name(gc).c_str());
                                isBattery = cls.find(L"Battery") != std::wstring::npos ||
                                            cls.find(L"battery") != std::wstring::npos;
                            }
                        }
                    }
                ...
                if (g_omniItemsPresenter)
                    g_omniItemsPresenter.ClearValue(FrameworkElement::HorizontalAlignmentProperty());
                g_omniStackPanel.ClearValue(StackPanel::OrientationProperty());
                g_omniStackPanel.ClearValue(FrameworkElement::HorizontalAlignmentProperty());
                int n = VisualTreeHelper::GetChildrenCount(g_omniStackPanel);
                for (int i = 0; i < n; i++) {
                    auto child = VisualTreeHelper::GetChild(g_omniStackPanel, i).try_as<FrameworkElement>();
                    if (child) {
    ...
    if (g_clockTimeTextBlock || g_clockDateTextBlock) {
        try {
            if (g_settings.verticalClock) {
                if (g_clockTimeTextBlock)
                    g_clockTimeTextBlock.TextAlignment(ClockTextAlignment());
                if (g_clockDateTextBlock) {
                    g_clockDateTextBlock.TextWrapping(TextWrapping::WrapWholeWords);
                    g_clockDateTextBlock.MaxWidth(80.0);
                    g_clockDateTextBlock.TextAlignment(ClockTextAlignment());
                }
            } else {
                if (g_clockTimeTextBlock)
                    g_clockTimeTextBlock.ClearValue(TextBlock::TextAlignmentProperty());
                if (g_clockDateTextBlock) {
                    ...
                    g_clockDateTextBlock.ClearValue(TextBlock::TextAlignmentProperty());
                }
            }
        } catch (...) {}
    }
