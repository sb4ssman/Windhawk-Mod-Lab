...
// Experiment 2:
// Built on experiment 1 and tested whether TextBlock justification was skipped
// because the clock line was the final paragraph line. `%s%` was resolved to a
// normal space, a trailing newline was appended, and WrapWholeWords was used.
// Live result: spacing/layout was worse and still not elastic.
...
std::wstring FormatLineToString(std::wstring_view format) {
    ...
    bool hasSpacer = formatted.find(kSpacerToken) != std::wstring::npos;
    formatted = ReplaceAll(formatted, kSpacerToken, L" ");
    if (hasSpacer) {
        // Experiment 2: TextBlock justification often skips the final line.
        // Add a trailing blank line so the visible spacer line is no longer final.
        formatted += L"\n";
    }
    return formatted;
}
